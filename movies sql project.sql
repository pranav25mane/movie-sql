...
select movie_name from movies_sql where budget>revenue order by (budget-revenue) desc  limit 5;
...
select * from movies_sql where lead_star='akshay kumar' and budget>revenue ;

#30.With which director Sharukh Khan have given the biggest hit movie .
select director from movies_sql where lead_star='shahrukh khan' order by revenue desc;
select * from movies_sql;
SELECT director
FROM movies_sql
